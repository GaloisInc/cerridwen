...
#    NAME, TARGET, POSITIVE, NEGATIVE, CORPUS, THRESHOLD
# where bound is 0 to run the evaluation with Gitz similarity score
# For each line, it performs the following:
# 1) cabal run cerridwen -- eval -t TARGET  -p POSITIVE -n NEGATIVE -e -s CORPUS --output NAME.csv --threshold N
# 1* if TARGET, POSITIVE, NEGATIVE, CORPUS are exactly "X", use the value in the first row
# 1** if NAME is exactly "X", use the value in the first row + THRESHOLD 
...
for row in data:
    ...
    cerridwenCall = "cabal run cerridwen -- eval --target " + target + " -p " + positive + " -n " + negative + " -e -s " + corpus + " --output " + name + ".csv -c --threshold " + str(bound)
